...
// fakeCache records Del calls; reads always miss.
...
func TestCacheInvalidator_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "member_added dels managers-of for user",
			subject:  events.SubjMemberAdded,
			payload:  `{"user_id":"` + userID.String() + `"}`,
			wantKeys: []string{cache.ManagersOfKey(userID)},
			acked:    true,
		},
		{
			name:     "manager_added dels managers-of for user",
		...
		{
			name:     "manager_removed dels managers-of for user",
		...
		{
			name:     "nil user_id: no Del, still acked",
